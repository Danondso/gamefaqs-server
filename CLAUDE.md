...
Migration v5 is the RAG bring-up: it adds the `chunks` table with `chunks_fts` + insert/delete triggers, the `chunks_fts_vocab` and `guides_fts_meta_vocab` fts5vocab views (read-only, used by `RetrievalService.filterToRareChunkTokens` to drop common tokens in O(log n) per token), the `guides.indexed_at` checkpoint column with composite index `(indexed_at, id)` for the indexer's prefetch cursor, and a one-shot relabel of existing guide titles to `${games.title}` (optionally `— ${author}`) with the old title stashed in `metadata.original_title`. The relabel is idempotent — rows with `original_title` set are skipped, so it's a no-op on fresh DBs and safe to re-run.

Migration v6 adds `games_fts` (FTS5 virtual table over `games.title`) plus insert/update/delete triggers on `games` to keep it in sync, and backfills from the existing `games` rows. Powers `RetrievalService.defaultGameMatch` — without it, "name the game" questions like "How do I beat the Elite Four in Pokemon Red?" had to rely on title-FTS, where the rare-token filter drops game-name tokens (`pokemon` df=675) in favor of action verbs (`beat` df=41) and surfaces the wrong games.
