...
Current `SCHEMA_VERSION` is **6**. Schema changes require a new migration in `src/database/migrations.ts`. The migration list was previously v1..v11; v6..v11 were collapsed into a single idempotent v6 (the WIP that introduced this lives on `mcp-fixes`). Existing DBs at v11 cannot boot directly against the collapsed code — `getCurrentVersion > SCHEMA_VERSION` throws — and need a one-time `DELETE FROM schema_version WHERE version > 5;` re-stamp so the collapsed v6 walks forward over the already-correct columns. The collapsed v6 is designed for that re-stamp: every CREATE uses `IF NOT EXISTS`, every ADD COLUMN routes through `addColumnIfMissing`, the FTS rebuild is gated on `guides_fts_meta` being empty, the chunk-provenance backfill is gated on `gamefaqs_id IS NULL`, and the scenario backfill UPDATEs are gated per-tag on `scenario IS NULL OR NOT LIKE '%tag%'`.

Migration v6 (collapsed) rolls in what was previously v6..v11 plus the `chunks.scenario` tag (a never-released v7 on this branch):
- Rebuilds `guides_fts_meta` triggers to index `games.title` (canonical game name) instead of `guides.title` (which after v5 is `${games.title} — ${author}`). Author bylines were leaking into title-FTS and causing false-positive matches on questions whose game-name tokens collided with author names (e.g., "Sephiroth" as a guide author surfaced unrelated guides). Drops and recreates the `guides_fts_meta_insert/_update` triggers to do a `games.title` lookup, adds a `games_title_propagate_to_guides_fts` trigger so renaming a game updates all linked FTS rows, and rebuilds the table from a JOIN (skipped on a re-stamped DB where `guides_fts_meta` is already populated). `guides.title` (and the Citation `guide_title` field) keeps the relabeled `${game} — ${author}` form for display.
- Adds chunk provenance columns (`gamefaqs_id`, `franchise`, `language`, `guide_author`, `guide_type`, `review_status`) and backfills them from `guides.metadata` / `games.metadata`. `IndexingService` writes these at insert time for new chunks.
- Adds `chunks.content_type` (NOT NULL DEFAULT `'prose'`) and `chunks.section_heading` for the type-aware chunker output.
- Adds the extraction tables (`canonical_game_groups`, `game_aliases`, `game_entities`) and the `games.canonical_group_id` column + indexes. **Seeding does NOT run here** — it moved to `EntitySeedService` (called from `InitService` post-import and from `server.ts` as a boot safety-net) because migrations run before the archive import populates `games`, so any seeding done at migration time would always insert zero rows. `EntitySeedService.needsSeed()` returns true when `games` is populated but `game_aliases` is empty.
- Adds `chunks.scenario` (TEXT, comma-joined tags from `{ng_plus, secret, missable}`) for soft-penalising gated content on plain questions. Backfill is content-only (no re-index, no re-embed) — three SQL UPDATEs match `content LIKE '%new game+%' / %easter egg% / %missable%` etc. and OR-merge tags into the column. New chunks get `scenario` set at chunk time by `Chunker.detectScenario` (same regex shapes as the SQL backfill). At retrieval time, `RetrievalService` applies a soft 0.7× penalty to chunks whose `scenario` is non-NULL when the question has no scenario cue (per `SCENARIO_CUE_RE`); the penalty is intentionally soft so gated chunks still surface when nothing better outranks them.
