...
import { chunkGuide, __testing, type ChunkOpts } from '../src/services/Chunker';
...
describe('chunkGuide', () => {
  ...
  it('forces a flush at a prose↔reference type shift (the Q7 KoritheMan fix)', () => {
    // Prose preamble followed by a KV stat block. The chunker must split
    // them so the embedding for the prose chunk doesn't get drowned out by
    // stat-block tokens.
